...
## Observability
The YAS observability stack follows the OpenTelemetry approach for traces and log correlation while using Prometheus-native scraping for metrics.
Promtail collects logs from application pods and sends them to the OpenTelemetry Collector. The collector forwards logs to Loki and traces to Tempo.
Prometheus collects application metrics through the ServiceMonitor resources generated by the YAS Helm charts.

View details configuration of Open Telemetry Collector at [opentelemetry](./observability/opentelemetry/values.yaml)
For the full telemetry flow, see [Kubernetes Observability Stack](../../docs/observability.md).
...
Loki also supports trace ID correlation. In Tempo, use Trace search and Node graph to inspect request flow.
